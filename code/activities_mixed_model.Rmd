...
```{r}
model_df <- pre_mm_df %>%
  select(-week_total_distance, -week_total_work, -week_average_power) %>%
...
model_df_scaled <- model_df %>%
  mutate_at(vars("week_total_workout_time":"rides_per_quarter"), scale)
```
...
#creating indices
trainIndex <- createDataPartition(model_df_scaled$critical_power,p=0.75,list=FALSE)

#splitting data into training/testing data using the trainIndex object
model_train <- model_df_scaled[trainIndex,] #training data (75% of data)

model_test <- model_df_scaled[-trainIndex,] #testing data (25% of data)
```
...
We can see that the variance in id is 174, which means that user id is very important and explains a lot of the variance, which we'll not be able to explain:
* training history
* genetics
* location
* performance level
...
Once we account for year and user id, there are still MANY statistically significant predictors of critical power. But when looking at the estiamte values, it appears to be that `coggan_np` and `week_relative_intensity` look to be 