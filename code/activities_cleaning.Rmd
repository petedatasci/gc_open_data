...
activities_total_rides <- activities_ride_no %>%
  ...
  mutate(rides_per_quarter = sum(rec)) %>%
  ungroup()
...
rides_weekly <- rides %>%
  group_by(id, year, week) %>%
  summarise(rides_per_week = n())

rides_quarterly <- rides %>%
  group_by(id,year,quarter) %>%
  summarise(rides_per_quarter = n())
...
```{r all-week-metrics}
weekly_metrics <- full_join(week_total_volume_2, week_avg_int, by = c("id", "year", "quarter", "week"))
```
...
quarter_metrics_2 <- quarter_metrics %>%
  left_join(rides_quarterly, by = c("id","year","quarter"))
...
ride_metrics <- full_join(quarter_metrics_2, cp_metrics, by = c("id","year","quarter")) 
